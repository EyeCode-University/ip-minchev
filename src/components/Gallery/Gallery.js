  ...
  return (
    <section id="gallery" className={styles.section}>
      <div className="containerWide">
        ...
        {/*
          Полный перечень изготовленных изделий. В сетке показываются четыре
          карточки, остальные позиции доступны только в лайтбоксе — то есть их
          названия и заводы-заказчики не попадали в разметку вообще. Здесь они
          присутствуют текстом: это и перечень для экранного диктора, и то, по
          чему сайт находят в поиске. Без <Image> намеренно — два десятка лишних
          изображений в DOM утянули бы LCP.
        */}
        <div className={styles.fullList}>
          <h3>Полный перечень изготовленных изделий</h3>
          <ul>
            {GALLERY_MEDIA.slice(PREVIEW_COUNT).map((item) => (
              <li key={item.src}>
